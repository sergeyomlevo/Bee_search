...
class RoomPhysicalObjectRepositoryTest {
    ...
    fun numberingIsIndependentByTypeAndTerritoryAndDesignationIsStable() = runBlocking {
        val firstHollow = repository.createHollow(territory1, 56.1, 42.7)
        val secondHollow = repository.createHollow(territory1, 56.2, 42.8)
        val logHive = repository.createLogHive(territory1, 56.3, 42.9)
        val otherTerritoryHollow = repository.createHollow(territory2, 56.4, 43.0)
    ...
    fun explicitBeeAssociationPersistsWithoutChangingFlightCycle() = runBlocking {
        ...
        val hollow = repository.createHollow(territory1, 56.1, 42.7)
