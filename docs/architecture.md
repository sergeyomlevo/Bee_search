...
В Room schema v1 внешние ключи и составные уникальности метки Bee и номера FlightCycle обеспечиваются ограничениями SQLite. Правила «одна активная ObservationPoint» и «один открытый FlightCycle на Bee» проверяются внутри транзакционных методов Repository; DAO остаются внутренней деталью слоя хранения. Это защищает операции приложения от конкурентной проверки и записи без добавления технических полей или таблиц.
...
confirmation
        ↓
ObservationPoint saved to Room
```
