...
Реализованная data/domain-основа долговечных физических объектов относится к
...
```text
apiaries.physical_object_id   UUID   PK, FK → physical_objects.id (RESTRICT)
...
```text
bees.source_object_id   UUID   nullable, FK → physical_objects.id (RESTRICT)
```
