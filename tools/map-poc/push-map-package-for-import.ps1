...
$remoteDirectory = "/sdcard/Download/BeeSearch/$PackageId"
...
$remoteHash = ((& $Adb -s $Serial shell sha256sum "$remoteDirectory/$PackageId.pmtiles" 2>&1) -split '\s+')[0].ToLowerInvariant()
...
Write-Host "Select $PackageId.pmtiles.manifest.json first, then $PackageId.pmtiles in Bee Search DEV."
